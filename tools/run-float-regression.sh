...
TEST_DIR=${MINIC_FLOAT_TEST_DIR:-"${REPO_ROOT}/tests/float_regression"}
...
MODE=${MINIC_FLOAT_TEST_MODE:-"all"}
...
TMP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/minic-float-regression.XXXXXX")
trap 'rm -rf "${TMP_DIR}"' EXIT

usage() {
	cat <<'USAGE'
Usage:
  ./tools/run-float-regression.sh
  ./tools/run-float-regression.sh <mode>
  ./tools/run-float-regression.sh <testcase>
  ./tools/run-float-regression.sh <mode> <testcase>
...
Modes:
  ...
  all    Run both ll and asm checks

Examples:
  ./tools/run-float-regression.sh
  ./tools/run-float-regression.sh ll
  ./tools/run-float-regression.sh asm float_truthiness

Environment:
  MINIC_BIN=./build/minic
  MINIC_FLOAT_TEST_DIR=./tests/float_regression
	MINIC_RUNTIME_SOURCE=./tests/sylib.c
	MINIC_RUNTIME_LIB=./tests/libsysy_riscv.a
  MINIC_FRONTEND=antlr|recursive|default
  MINIC_FLOAT_TEST_MODE=ll|asm|all
  MINIC_FLOAT_LL_OPT_LEVEL=0|1
  MINIC_FLOAT_ASM_OPT_LEVEL=0|1
  CLANG_BIN=clang
...
if ! validate_opt_level "${LL_OPT_LEVEL}"; then
	fail_with_usage "Unsupported MINIC_FLOAT_LL_OPT_LEVEL: ${LL_OPT_LEVEL}"
fi

if ! validate_opt_level "${ASM_OPT_LEVEL}"; then
	fail_with_usage "Unsupported MINIC_FLOAT_ASM_OPT_LEVEL: ${ASM_OPT_LEVEL}"
fi
...
if [[ ! -d "${TEST_DIR}" ]]; then
	fail_with_usage "Float regression directory not found: ${TEST_DIR}"
fi
...
MODE=$(normalize_mode "${MODE}") || fail_with_usage "Unknown MINIC_FLOAT_TEST_MODE: ${MODE}"
