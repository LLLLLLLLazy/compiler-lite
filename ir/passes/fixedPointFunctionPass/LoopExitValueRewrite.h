...
///
/// 对非负常量初值、步长为 1 的规范计数循环，把循环头部满足以下递推形式
/// 的 phi 在循环出口处的取值替换为闭式表达式：
///   1. 仿射递推      p_{k+1} = p_k + c            -> p_exit = start + c * N
///   2. 2 的幂除法递推 p_{k+1} = p_k / 2^s          -> p_exit = start sdiv 2^(s*N)
///   3. 二次递推      p_{k+1} = p_k + a*i_k + b     -> 按辅助仿射 IV 的等差和求值
/// 其中 c、start、a、b 均为循环不变量
/// 2 的幂除法递推将循环内反复的除法收敛为一次变量移位（带符号修正），消除内层除法循环。
///
